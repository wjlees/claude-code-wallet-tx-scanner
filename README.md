...
└── modules/
    ...
    │   │   ├── ethereum-common.service.ts # assetId당 1인스턴스. scanTransactions=블록범위 2-phase(getBlock hydrated+매칭건 receipt status===1, value>0), confirmations cap
    │   │   ├── evm-batch.ts              # jsonRpcBatch: JSON-RPC 배열 batch + 청크 Promise.all 병렬(블록/receipt fetch)
...
- **2026-07-01**: **입금 감지 정확도 — status/value 필터 + confirmations cap + EVM batch** (scanBlocks 정합, §5-11). (1) **EVM 네이티브**: `getBlock(n,true)` hydrated 로 from/to·`value>0` 후보 거르고 **매칭 tx 만 `getTransactionReceipt` → `status===1`**(2-phase; revert 값이동 방지). (2) **EVM 토큰**: revert 로그는 `eth_getLogs` 에 안 나와 로그 존재=성공 → receipt 불필요, `data≠0` 필터만. (3) **confirmations cap**: `to=min(from+range-1, head-confirmations)` 로 최근 블록 제외(스냅샷 저장 X), `getConfirmations(path)`(미설정 0). (4) **성능**: `evm-batch.ts` `jsonRpcBatch`(JSON-RPC 배열 batch + 청크 Promise.all 병렬)로 블록/receipt fetch. 검증: konet 부팅 — `confirmations=12`→native `safeHead=head-12`, 블록 batch fetch 정상. **EVM 먼저 적용, 타 체인은 같은 원칙 확장 TODO.**
