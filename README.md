...
└── modules/
    ...
    │   ├── parameter-store.ts       # getParametersByPath(path) — path별 {nodeUrl,maxDepositScanRange,rawDecimal} 조회 (parameter.json, DB 교체 예정)
    │   ├── asset.repository.ts      # main.asset 메타 read port(§19) — confirm_threshold 조회 (stub)
    │   ├── asset-repository.module.ts # ASSET_REPOSITORY provider — confirm_threshold 쓰는 체인 모듈들이 import
    │   ├── crypto-util.ts           # 블록체인 공용 유틸. toSatoshi(raw, rawDecimal) — BigNumber 로 사토시(8자리) 환산(버림)
    ...
    │   ├── sui/                      # AssetService. gRPC 단독(@mysten/sui/grpc). checkpoint 범위 스캔, balance_changes delta
    ...
    └── tx-scanner/                  # 핵심: 무한 루프 주체 + tx 감지 + 저장
        ...
        ├── unspents.repository.ts    # wallet_scanner_unspents(UTXO 원장, §17) 인터페이스 + in-memory stub — 잔고 SUM/vin 출금탐지
    ...
    scripts/
    └── btc-utxo-scan-test.mjs        # BTC/BCH 노드에 붙여 UTXO 스캔(§15/§17) 로직을 미리 실행해보는 독립 테스트(의존성 0)
```
...
- **2026-07-10**: **SUI 처리량/429 대응** — checkpoint 생성 ~4.5/s 실측 → sui `maxDepositScanRange` 10→**100**(10이면 영구 뒤처짐). `CHECKPOINT_CHUNK=25` 청크 병렬(청크 간 순차, 실패 시 이후 청크 생략). 공용 엔드포인트 429 실측: 부분 429 는 연속 성공 구간 전진으로 유실 0(96/100), getServiceInfo 429 는 러너가 다음 사이클 재시도. 자체 fullnode 는 제한 없음. per-tx 조회 제거로 사이클당 콜 수 = checkpoint 수(+1) — tx 수와 무관.
...
- **2026-07-16**: **UTXO watch 정책 + scantxoutset 백필(§5-23)**. 비-UTXO=포워딩 모델(핫/콜드 중심), **UTXO=유저 입금주소 전체 watch**(포워딩 없음·vin 직접 출금). `WalletService.getScanAddresses` 가 UTXO 자산이면 `getUserDepositAddresses`(stub) 를 합침. `TxScannerService.backfillUnspents(assetId)` — scantxoutset(1000주소 청크 순차)으로 살아있는 UTXO 를 unspents 테이블에 멱등 적재(기존 잔고 주소 추가 시 일회성 프로비저닝, monorepo=HTTP 엔드포인트). btc/bch 서비스에 `listLiveUtxos` 노출.
- **2026-07-16**: **잔고대사(Reconciliation) 시스템 개요 문서** [docs/RECONCILIATION.md](./docs/RECONCILIATION.md) — 용어(DB잔고/온체인잔고/온체인TX), 만들 것 5개(스냅샷+block_number, 시점 보정, 불일치 감지·알림/리포트, 제외 tx 등록, 대사 웹), 헬스체크 등식(실측 vs 직전 실측+온체인TX = 스캐너 완전성 검증), 전체 흐름. 설계 확정 시 이 문서에 구체화.
...
- **2026-07-02**: **SOL/SPL from/to·amount 잔고 delta 파싱** (§5-16). SOL/SPL 은 tx 에 금액 필드가 없어 `getParsedTransaction` 잔고 delta 로 계산 — SOL=`pre/postBalances` lamports delta(fee-payer delta 는 fee 분리), SPL=`pre/postTokenBalances`(해당 mint) owner 별 delta. **최대 감소=from, 최대 증가=to/amount**. SPL 0금액 skip, SPL 에 parsed 호출 추가(SOL 은 fee 작업 때 이미 호출). **tx_index=0 확정**(delta=tx 단위 합산→tx당 1행, instruction 인덱스 교체 계획 폐기). 검증: 실 메인넷 3 tx from/to/amount/fee 정확 파싱, build/lint/부팅.
- **2026-07-02**: **UTXO 출금(vin) 감지 + fee** (§5-15). vin prevout(값·주소)을 **`getblock` verbosity 3**(Core 25+, `vin.prevout` 인라인, 추가 콜 0)으로 받아 vin∈watch면 출금→ **fee=Σvin−Σvout** native 행(§14). 미지원 노드는 vin마다 `getrawtransaction` fallback. 설정 플래그 `UtxoAssetConfig.prevoutInline`(btc=true, bch=false). 수신(vout)은 기존대로, fee 행 tx_index=vout 개수(비충돌). rule 1 순수(노드 RPC만). 개선사항으로 `wallet_scanner_unspents`(O(1) 탐지, 캐치업 최적화)·change 판별·import 백필 등록. (BTC/BCH 노드 미설정이라 런타임 미검증, build/lint 통과.)
- **2026-07-01**: **fee 모델 = native 자산 별도 행 + 방향별 스캔 + 실패tx status** (§5-14). 수수료는 항상 native 코인이라 **토큰 행엔 fee 안 넣고, 별도 native 행**(amount=0, fee=gas)으로 기록 — 한 tx 가 여러 asset_id 행(ERC20 → 토큰 행 + ETH fee 행), 유니크 키로 공존. **native 스캔 방향별**: `tx.from∈watch`(originate=fee 소모)면 value·status 무관 기록(**실패 tx 도 status=0**), `to∈watch`(수신)이면 value>0·성공만. **`fee_amount` 는 fee-payer∈watch(우리가 낸 것)일 때만**(EVM=tx.from, XRP=Account, XLM=source_account, XPLA=sender). `DetectedTx.status`(1/0) 추가. prototype: EVM 네이티브(방향별+status+fee 조건부)·XRP/XLM/XPLA fee 조건부. TRX/UTXO/SOL·토큰 fee 는 native 행 관점 후속(추가 fetch). 검증: build/lint/부팅.
- **2026-07-01**: **amount·fee 사토시 통일 + raw_amount/raw_fee_amount** (§5-13). DB `amount`·`fee_amount` 를 **사토시(8자리 정수)**로 — `blockchain/crypto-util.ts` `toSatoshi(raw, rawDecimal)`(`BigNumber.shiftedBy(8-rawDecimal).floor`, 버림). 원본은 **`raw_amount`·`raw_fee_amount`** 보존(lossy). **fee 는 항상 native(base) 자산 scale**(코인=자기 rawDecimal, 토큰=기반 코인 rawDecimal). 환산은 tx-scanner `toInsertParams`. **rawDecimal**: 코인=ParamStore `rawDecimal`(`AssetService.getRawDecimal()`), 토큰=`main.token.token_decimal`. blockchain 은 raw 정수 반환(UTXO=`vout.value` BTC소수→satoshi 정수). **fee 파싱**: EVM 네이티브(gasUsed×effectiveGasPrice)·XRP(`tx.Fee`)·XLM(`fee_charged`)·XPLA(`auth_info.fee`) 완료, TRX/UTXO/SOL/SPL·토큰 fee 는 후속(추가 fetch/parse). `bignumber.js` 의존성(소수/큰 수는 BigNumber — CLAUDE.md §6). 검증: toSatoshi(1.5ETH→150000000, 12.345678XRP→1234567800, sub-satoshi 버림), 부팅 무에러.
- **2026-07-01**: **입금 감지 정확도 — 비-EVM 확장** (§5-12). §5-11(EVM)을 나머지 체인 `scanTransactions` 로. **status(성공) 필터**(체인별): UTXO=불필요, TRX/TRC20=`contractRet==='SUCCESS'`(TRC20 은 call data 디코드라 필수), XRP=`tesSUCCESS`, XLM=`successful`, XPLA=`code===0`, SOL/SPL=`sig.err===null`. **confirmations cap**: 블록높이 기반(UTXO/TRX/TRC20/XPLA)은 `head-confirmations`, SOL/SPL 은 commitment `finalized`, XRP/XLM 은 즉시확정. **0금액 필터** 전 체인. 검증: 부팅 시 전 비-EVM 스캔 정상(로직 에러 없음). monorepo 는 비-EVM scanTransactions 에 동일 적용 TODO.
