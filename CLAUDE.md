...
- **confirmations (reorg 안전 마진)**: `getConfirmations(path)`(미설정=0). 스캔 끝을 `head` 가 아니라 **`safeHead = head - confirmations`** 로 캡한다(`to = min(from+range-1, safeHead)`, `from > safeHead` 면 skip). 이러면 감지된 tx 는 항상 ≥confirmations 확정 → **confirmations 를 저장/추적할 필요 없음**(스냅샷 X). cursor=null 첫 사이클은 `from=safeHead` 로 시작. **적용: EVM(native/token). 다른 체인은 같은 원칙으로 확장 예정(TODO).**
- **입금 감지 정확도 필터는 blockchain 층에서**(파싱=실제 일어난 transfer): EVM **네이티브**는 revert 시 블록엔 있어도 값이 안 움직이므로 **receipt `status===1` 확인 필수** + `value>0`. fetch 는 `getBlock(n,true)`(hydrated, from/to/value) 로 후보 필터 후 **매칭 tx 만 `getTransactionReceipt`** (2-phase; getTransaction 불필요). EVM **토큰**은 **revert tx 의 로그는 `eth_getLogs` 에 안 나오므로**(로그 존재=성공) receipt 불필요 — `data≠0`(0금액) 필터만. 성능: 블록/receipt 는 **JSON-RPC batch + Promise.all 병렬**(`ethereum-common/evm-batch.ts` `jsonRpcBatch`). confirmations threshold·금액 scale·in/out 은 위층(tx-scanner) 몫(§5).

## 3-2. 노드 연동 규칙
- 노드 URL·파라미터는 **`parameter-store.ts` 의 async `getParametersByPath(path)`** 모델로 읽는다(monorepo ParamStore 동일). 헬퍼: `getNodeUrlByPath(path)`, `getMaxDepositScanRange(path)`. 소스: 루트 `parameter.json`(자산별 path 객체 `{ nodeUrl, maxDepositScanRange }`). `parameter.json` gitignore(실값), `parameter.example.json` 템플릿. 노드 미설정이면 스캔 skip + `warnMissingNode`. **path 는 monorepo path 와 일치**(예: trx→`tron`, klaytn→`klay`, base(BASEETH)→`base`; 매핑 표 docs/MONOREPO-MAPPING.md §2). 추후 DB/원격 ParamStore 로 교체해도 호출부는 안 바뀐다.
