...
- **스캔 진행 지점 영속화(`startBlockNumber`)**: 과거 "cursor" 개념의 저장값. `ScanRunner` 는 시작 시 `AssetRepository.getStartBlockNumber(assetId)` 로 마지막 지점을 읽고, 사이클마다 `updateStartBlockNumber(assetId, …)` 한다(재시작/워치독 재가동 시 이어서, at-least-once). 저장 위치는 **`asset` 테이블의 `start_block_number` 컬럼**(행 키 = `resolveStoreAssetId(target)`, 토큰은 토큰 자체 assetId). 체인 경계(`scanTransactions`)의 반환값은 여전히 불투명 `cursor`(블록번호/signature/ledger 등)이고, `ScanRunner` 가 이를 `startBlockNumber` 로 받아 저장한다. 현재 구현은 `StubAssetRepository`(in-memory) — `ASSET_REPOSITORY` 토큰 주입, 실제 DB(`asset` 테이블)로 교체 시 인터페이스만 유지. **stub 은 in-memory 라 프로세스 재시작 시 진행 지점이 초기화된다(실제 DB 연결 시 영속).**
- **tx 저장**: `TxRepository.saveMany(target, txs)` 는 `mapDetectedTxToInsertParams(target, tx)`(detected-tx.mapper)로 변환 후 `DetectedTransactionsRepository.insertDetectedTransactions`(테이블 `detected_transactions`)로 저장한다. 현재 `StubDetectedTransactionsRepository`(콘솔) stub — `DETECTED_TRANSACTIONS_REPOSITORY` 토큰 주입, 실제 DB 로 교체 시 인터페이스만 유지. 매핑: direction 으로 from/to, `txHash→txId`, `memoId→note`, `amount ?? '0'`(NOT NULL); 저장 assetId 는 자산=코인 assetId, **토큰=토큰 자체 assetId**(현재 stub 1001+, 실제는 DB token→assetId). 멱등 `(assetId, txId, from/toAddress)` 는 DB 단계 TODO.
- **scan range**: 1회 스캔 처리량(블록수/page limit)은 `parameter-store` 의 `getMaxScanRange(<ASSET>_MAX_SCAN_RANGE)` 로 조회한다(cursor 는 절대 지점 유지, range 만 조절). **필수값** — 코드 default 없음, 미설정/0이하면 **throw**. 각 서비스가 `onModuleInit` 에서 노드 핸들과 함께 조회해 state 에 저장하므로, 노드는 있는데 range 가 없으면 그 자산은 초기화 실패(조용한 fallback 금지). 노드 미설정 자산은 range 도 불필요(skip). 권장값: EVM/SOL=1000, XLM/XRP=200, BTC/BCH=50, TRX/TRC20=20, XPLA=10 (parameter.json 에 명시).
...
- SDK: EVM=`web3`, SOL/SPL=`@solana/web3.js`, XLM=`@stellar/stellar-sdk`(Horizon), BTC/BCH=bitcoind JSON-RPC(fetch), **TRX/TRC20=`tronweb`, XPLA=Cosmos LCD REST(fetch, `XplaRestClient`), XRP=rippled JSON-RPC(fetch)**. EVM 토큰은 기반 자산의 web3, trc20 은 trx 의 tronweb 을 재사용한다(새 connection 만들지 않음).
...
- ⚠️ **ESM/CJS 제약**: 빌드가 CommonJS 라 ESM-only deps 와 충돌한다. `@stellar/stellar-sdk` 는 **12.x 고정**(16.x = ESM `@noble/hashes@2` require 실패), `@solana/web3.js` 는 `overrides` 로 `rpc-websockets`→`uuid@9.0.1` 고정. **`xrpl`(XRP 공식 SDK)도 `@noble/hashes@2`(ESM) 를 강제해 모든 버전이 `ERR_REQUIRE_ESM` → SDK 대신 rippled JSON-RPC(fetch) 사용.** **`@xpla/xpla.js` 는 top-level import 만으로 부팅 크래시(`@xpla/xpla.proto` v2 누락) → 제거하고 Cosmos LCD REST(fetch, `XplaRestClient`) 사용.** `tronweb` 은 CJS 정상. 버전 올릴 때 `npm run build` + 부팅으로 `ERR_REQUIRE_ESM`/크래시 재발 여부 확인.
