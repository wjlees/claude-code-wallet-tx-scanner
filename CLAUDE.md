...
- blockchain 서비스는 코인 `scanTransactions(addresses, cursor)` / 토큰 `scanTransactions(addresses, cursor, contractAddresses[]) → { txs, nextCursor }` 만 제공한다. **어떻게 스캔할지는 자산이 결정** — **전 자산 = 범위 스캔 + 자산당 커서 1개**(EVM/BTC/TRX/TRC20/XPLA=블록범위, SOL+SPL=슬롯 범위(§22 통합), XLM=ledger `payments`, XRP=ledger(validated) 전체 tx, SUI=checkpoint 범위). ⚠️ 주소별 스트림 조회(getSignaturesForAddress/account_tx/Horizon forAccount)는 **주소별 커서가 없으면 유실**(공유 커서가 다른 주소의 미처리분을 지나침)이라 채택하지 않는다(§22 결정). cursor 는 자산별 불투명 문자열. 토큰은 `contractAddresses`(이 token_type 의 토큰들)로 그 컨트랙트들의 transfer 만 수집하고, **각 tx 에 `contractAddress` 를 채워** 어느 토큰인지 분류 가능하게 한다(EVM=`address` 필터+`log.address`, SPL=`mint`, TRC20=`contract_address`).
...
- **`ScanTarget` both-set(assetId+tokenTypeId 둘 다) = "통합 러너"(§22, SOL+SPL 구현됨)**: **tx-scanner 내부 상수 `UNIFIED_SCAN_PAIRS`**(러너 구성은 tx-scanner 책임 — blockchain 층은 코인 서비스가 contractAddresses 를 받는 능력만 제공)에 등록된 쌍은 코인/토큰 개별 러너 대신 **both-set 러너 1개**를 만든다. SOL+SPL 이 대표 — 블록(getParsedBlock) 하나에 native delta(pre/postBalances)와 SPL delta(pre/postTokenBalances)가 함께 있어 **한 번 파싱으로 SOL 행 + SPL 토큰 행(contractAddress=mint)을 동시 생성**(블록 이중 다운로드 방지). 코인 서비스(SolService)가 옵션 `contractAddresses` 를 받아 둘 다 반환하고, tx-scanner 는 행의 contractAddress 유무로 코인/토큰 저장을 분리, 진행지점은 코인 행+토큰 행들을 같은 값으로 갱신(§10). SplService 의 ATA signature 스캔은 단독 운용 fallback 으로만 유지. 반면 **EVM 은 감지 RPC 가 달라(native=블록/receipt, erc20=getLogs) 합칠 수 없다**.
...
- **UTXO 원장(`wallet_scanner_unspents`, wallet DB — 매핑 §17)**: UTXO 는 노드 RPC 로 주소 잔고를 못 구하므로 감시 주소의 UTXO 를 테이블로 유지한다(잔고=`SUM(usable=1)`, vin O(1) 출금탐지, 출금 vin 조합 재료). **유지는 tx-scanner**(`maintainUnspents`): UTXO 스캐너가 `DetectedTx.utxo{created, spentOutpoints}` 로 알려주면 수신=INSERT(usable=1, `(asset_id,tx_id,tx_index)` 유니크 멱등)/소비=usable=0+spent_tx_id. blockchain 은 DB 안 봄(rule 1). **백필**(잔고 있는 주소 추가 시)은 `TxScannerService.backfillUnspents(assetId, addresses?)`(§23 — scantxoutset 청크 순차, 멱등 insert; monorepo 는 HTTP 프로비저닝 엔드포인트로 노출) 일회 실행 — 블록 1~latest 재스캔 금지(비현실적).
- **scan range**: 1회 스캔 처리량은 `parameter-store` 의 `getMaxDepositScanRange(path)`(= `getParametersByPath(path).maxDepositScanRange`)로 조회. 필드명은 **`maxDepositScanRange`**(monorepo ParamStore 실제 키와 일치 — `maxScanRange` 아님). 코드 default 없음. **미설정/0이하면 `undefined` 반환 → 노드 미설정과 동일하게 log 후 skip(throw 안 함, 앱 부팅 계속)**. 각 서비스가 `onModuleInit` 에서 노드 URL+range 둘 다 있을 때만 핸들 초기화(둘 중 하나 없으면 그 자산 루프만 조용히 skip). 권장값: EVM=1000, SOL=100(슬롯 — 생성 ~2.5/s 의 4x), SUI=100(checkpoint — 생성 ~4.5/s 의 2x), XLM/XRP=200(ledger), BTC/BCH=50, TRX/TRC20=20, XPLA=10 (parameter.json path 별).
- **confirmations (reorg 안전 마진)**: `getConfirmations(path)`(미설정=0). 스캔 끝을 `head` 가 아니라 **`safeHead = head - confirmations`** 로 캡한다(`to = min(from+range-1, safeHead)`, `from > safeHead` 면 skip). 이러면 감지된 tx 는 항상 ≥confirmations 확정 → **confirmations 를 저장/추적할 필요 없음**(스냅샷 X). cursor=null 첫 사이클은 `from=safeHead` 로 시작. **블록높이 기반(EVM/UTXO/TRX/TRC20/XPLA)에 적용.** 즉시확정 체인은: XRP=**validated ledger 만 스캔**(`ledger_current` 는 미검증이라 사용 금지)·XLM=ledger close·XPLA=commit 이라 보통 0, **SOL/SPL 은 numeric cap 대신 commitment `finalized`** 로 대체.
...
## 5. 도메인 메모
- **watch(감시 주소) 정책(§23)**: 비-UTXO 체인은 입금이 즉시 핫월렛으로 **포워딩**되므로 핫/콜드(+거래소 계정) 중심으로 충분하다. **UTXO(BTC/BCH)는 포워딩 없이 유저 입금주소에 UTXO 가 머물고 출금도 그 UTXO 들을 vin 으로 직접 조합**(핫월렛 경유 없음)하므로 **유저 입금주소 전체**를 watch 에 포함해야 한다(`WalletService.getUserDepositAddresses` — monorepo 는 유저 입금주소 테이블 조회+캐싱). 출금 관측은 unspents 테이블의 UTXO 가 vin 으로 소비되는지로 충분.
...
- 자산별 특성: EVM=nonce/계정·블록범위, BTC/BCH=UTXO, XLM=memoId 입금 식별(+금액은 tx 가 아니라 **payment operation** 에 있음 — ledger `payments` 스캔), XRP=DestinationTag+**`meta.delivered_amount` 가 금액의 진실**(partial payment 방어 — tx.Amount 로 크레딧 금지), XPLA=Cosmos transfer 이벤트(amount 는 "1234axpla" denom 붙은 문자열 — **axpla 만 추출**, 타 denom 오인 금지), SOL/SPL=mintAddress·슬롯 블록 스캔(주소 수 무관). 이 차이를 blockchain 서비스가 캡슐화한다.
